...
def home():
    ...
    if request.method == 'POST':
        ...
        condition_encoded = CATEGORICAL_DATA['Condition'][condition]
        place_encoded = CATEGORICAL_DATA['Place'][selected_place]
        X = np.array([temperature, condition_encoded, pressure_hpa, humidity, uv_index, rain, place_encoded])
        X = X.reshape(1, -1)
        y = model.predict(X)
